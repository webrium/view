...
class Engine
{
    ...
    protected static function evaluateCompiledFile(string $compiledPath, array $data, string $originalView = ''): string
    {
        ...
        set_error_handler(function (int $errno, string $errstr, string $errfile, int $errline) use ($originalView) {
            $e = new ViewTemplateException($errstr, $errno);

            $headerLines = self::COMPILED_HEADER_LINES;
            $realLine = max(1, $errline - $headerLines);

            throw (new ViewException(
                $errstr,
                $errno,
                new \ErrorException($errstr, $errno, $errno, $errfile, $realLine)
            ))->setOriginalView($originalView ?: $errfile);
        });
